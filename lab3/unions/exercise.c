 ...
 * you will set the integers to 6 given values and then
 * print out the character array both as a series of chars and as a string.*/
    ...
    int main() {
        ...
        union united intCharacters = {{1853169737, 1936876900, 1684955508, 1768838432, 561213039, 0}};
      
        /* testing code */
        printf("[");
        // only go to 18 because 1 byte is for the terminating 0 and we don't print the last in the loop
        for(int i = 0; i < 19; ++i)
            printf("%c, ", intCharacters.symbols[i]);
        printf("%c]\n", intCharacters.symbols[19]);
    
        printf("%s\n", intCharacters.symbols);
    }
